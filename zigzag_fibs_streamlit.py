...
with col1:
    epsilon = st.slider('Epsilon', min_value=0.01, max_value=0.5, value=0.1, step=0.05) # Moved epsilon slider to main panel
with col2:
...
if data is not None:
    ...
    with col2_window:
        window_width = st.slider('Window Width', min_value=50, max_value=len(data) if len(data) > 100 else 100, value=100, key='window_width_2') # Adjusted max_value for window_width
        
    max_start_window_index = max(0, (len(data) - window_width) // window_width) # Recalculate max_start_window_index based on current 
    with col1_window:
        start_window_index = st.slider('Start Window', 0, max_start_window_index, 0, key='start_window') # Slider for window multipliers, default to 0window_width
    ...
    highs = data.high.values # Use wdata
    lows = data.low.values # Use wdata

    # Calculate ZigZag on windowed data
    high_low_markers, turning_markers = calculate_zigzag(highs, lows, epsilon=epsilon) # Use whighs and wlows
    ...
    running_highs = pd.Series(np.where(high_low_markers == 1, 1, np.nan) * highs).ffill().values
    running_lows = pd.Series(np.where(high_low_markers == -1, 1, np.nan) * lows).ffill().values
    ...
    wdata = get_window_data(data, start_window_index, window_width)


    if not wdata.empty: # Proceed only if wdata is not empty
        whighs = wdata.high.values
        wlows = wdata.low.values
        ...
        candlestick_ohlc_args={'width': 1 / np.log(len(wdata)) if len(wdata) > 1 else 0.6} # Avoid log(0) error
        ...
        wturning_points_ix = turning_points_ix[(turning_points_ix >= start_window_index) & (turning_points_ix <= start_window_index + window_width - 1)]
        wextreme_points_ix = extreme_points_ix[(extreme_points_ix >= start_window_index) & (extreme_points_ix <= start_window_index + window_width - 1)]
        wextreme_points = high_low_markers[wextreme_points_ix]
        wextreme_prices = np.where(wextreme_points == 1, whighs[wextreme_points_ix], wlows[wextreme_points_ix]) 


        ax1.plot(wdata.index[wextreme_points_ix], wextreme_prices, color='purple', label='ZigZag Line', lw=1.5, alpha=0.8) # Use wdata.index
        for ix in wturning_points_ix:
            ax1.axvline(wdata.index.values[ix], color='gray', linestyle='-', alpha=0.2, lw=1) # Use wdata.index
